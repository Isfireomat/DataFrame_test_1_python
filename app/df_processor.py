...
class DFProcessor:
    ...
    @staticmethod
    def _get_value(data: pd.DataFrame, df_id: int, year: int, field_name: str, prev: int, last_available: int) -> float:
        columns = list(filter(lambda col: col in data.columns, list(map(lambda y: f"{y}, {field_name}", range(year - prev, year - last_available - prev - 1, -1)))))        
    ...
    def get_data(self, data: pd.DataFrame, applications: pd.DataFrame, queries: list[dict[str, any]]) -> pd.DataFrame:
        ...
        def process_query(query):
            ...
            methods = {
                "Прибыль (убыток) от продажи, RUB": lambda row: self._calculate_profit(data, row['_id'], row['year'], prev, last_available),
                "Возраст компании, years": lambda row: self._get_company_age(data, row['_id'], row['year']),
                "": lambda row: self._get_value(data, row['_id'], row['year'], field_name, prev, last_available)
            }
            method = methods.get(field_name, methods[""])
            
            postfix = f"{('Prev' if prev > 0 else 'Next') * abs(prev)} {'LA' + str(last_available) if last_available else ''}".strip()
